...
### 1-2. グローバルセカンダリインデックス (GSI) の作成（リーダーボード用）

各ゲームごとのランキング（リーダーボード）を取得するために、インデックスを追加します。

1. 作成したテーブル (`MusicToysDB`) をクリックして詳細を開きます。
2. 「インデックス」タブを選択し、「インデックスの作成」をクリックします。
3. **パーティションキー**: `gameId` (文字列)
4. **ソートキー**: `score` (数値)
5. **インデックス名**: `LeaderboardIndex` (自動入力されない場合は入力)
6. **射影された属性**: `All` (すべての属性) ※または `Include` で `name`, `timestamp`, `level` などを選択
7. 「インデックスの作成」をクリックします（作成完了まで数分かかる場合があります）。
...
2. **HTTP API** を選択して「構築」。
3. **統合を追加**: 作成したLambda関数 (`MusicToysBackend`) を指定。
4. **API名**: `MusicToysAPI` (任意)
5. ルートの設定:
...
2. バケット名を入力します（世界で一意である必要があります。例: `music-toys-2025`）。
